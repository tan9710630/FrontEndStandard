...
目前这些规范一部分来源是**王子荣**提供的`JS`编码规范，一部分来源是VUE风格指南，还有一部分是我定义的基础规范
...
目前我们允许在项目开发过程之中（开发环境与测试环境）关闭`no-console`与`no-debugger`规则，但是生产环境必须开启。一旦后续再生产环境之中看到`console.log`代码那就是最大恶极。
...
这里千万使用驼峰法命名 class 的属性。以下是一些常用到的 class的名字。
...
使用 ES6 的方式引入。如果它们有依赖关系，最好体现在命名上，比如：
...
简单来说就是**v-if**和 **v-for**不能放在同一个标签里面，一定要分开
...
这里的命名与style的class命名风格一致，必须采用 `@foo-bar` 形式，不允许使用 `@fooBar` 形式。
...
> 无论是视图组件还是部分自定义的公用组件除开内部基础的逻辑代码之外，有可能会存在大量的常量定义，外部逻辑。之前大量的组件封装都直接写在VUE文件的`script`里面，这样一来导致vue文件内部极其臃肿，可读性极差。
